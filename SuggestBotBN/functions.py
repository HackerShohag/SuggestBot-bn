...
def postwarn(username,lastposttime=None,warncode=0):
	elapsedtime = (time.time() - lastposttime)/(60*60)
	warnlist = []
	for i in range(0,10):
		filename = 'warning0'+str(i)
		try:
			with open(filename,'r') as file:
				warns = file.read().replace(r'{{time}}',elapsedtime)
				warnlist.append(warns)
		except Exception:
			pass
	page = pywikibot.Page(pywikibot.Site('bn','wikipedia'),username)
	page.text = page.text.__add__(warns[warncode])
...
def postRec(username,recmsg,note,frequency=None,update=None):
	...
	#addint note if exists
	if note:
		finishing = note.__add__(suggestbotsign())
	...
	if page.exists():
		if update_and_replace == True:
			page.save(u'পরামর্শবট কর্তৃক নিবন্ধ হালনাগাদ করা হল ')
		else:
			page.save(u'পরামর্শবট কর্তৃক নিবন্ধ প্রস্তাবনা প্রেরণ ')
...
def pagedata(page):
	...
	res = requests.get('https://bn.wikipedia.org/w/index.php?title=' + page + '&action=edit&undoafter=0&undo=0')
	...
	alltags = re.findall(r'<a.*?>টেমপ্লেট:(.+?)</a>',unescape(res.text))
	...
	for k in range(0,1):
		list2 = re.findall(r'{{.*?অসম্পূর্ণ.*?}}|{{.*?অসমাপ্ত.*?}}|{{.*?অসম্পুর্ণ.*?}}|{{.*?stub.*?}}|\[\[বিষয়শ্রেণী:.*?অসম্পূর্ণ.*?\]\]|\[\[বিষয়শ্রেণী:.*?অসম্পুর্ণ.*?\]\]',text)
		if list2:
	...
	encat1 = re.findall(r'\[\[Category:(.*?)\]\]',text)
	encat2 = re.findall(r'\[\[category:(.*?)\]\]',text)

	catlist = bncat + encat1 + encat2
...
def getRandomList(cursor,usercontribcat=None,usercat=None):
	...
	if usercat:
		DB1 = []
		for cat in usercat:
			cursor.execute('SELECT * FROM Pages WHERE Categories LIKE "%' + cat + '%";')
			usercat_db = cursor.fetchall()
			cursor.execute('SELECT * FROM Pages WHERE PageName LIKE "%' + cat + '%";')
...
def recMsg(pagenames,details=None):
	...
	pagedata = '\n==নিবন্ধগুলি আপনার পছন্দ হতে পারে, পরামর্শবটের পরামর্শ==\n[[ব্যবহারকারী:পরামর্শবট|পরামর্শবট]] থেকে আপনাকে এই নিবন্ধগুলি সম্পাদনা করার জন্য পরামর্শ দেয়া হলো। আশা করছি আপনার পছন্দ হবে। শুভ সম্পাদনা!\n'+ table_header
...
def suggestbotsign():
	date = datetime.datetime.utcnow().strftime("%H:%M, %-d %B %Y (ইউটিসি)")
	...
	for i in range(0,12):
		if i<10:
			date = date.replace(enum[i],bnum[i])
		date = date.replace(emonths[i],bmonths[i])
	sign = '--[[ব্যবহারকারী:পরামর্শবট|পরামর্শবট]] ([[ব্যবহারকারী আলাপ:পরামর্শবট|আলাপ]]) '.__add__(date)
	return sign
...
def main(cursor):
	...
	for candidate in candidates:
		...
		if shouldPost(frequency,lastposttime) == True:
			...
			postRec(candidate,rec,note,frequency=frequency,update=update)
